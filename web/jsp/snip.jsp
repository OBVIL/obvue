...
<html>
  <head>
    <meta charset="UTF-8">
    <title>Recherche, <%=alix.props.get("label")%> [Obvie]</title>
  ...
  <body class="results">
    ...
    // TODO: merge these two blocks
      ...
      for (int i = 0; i < len; i++) {
        ...
        if (sort != OptionSort.score) href.append( "&amp;sort=").append(sort.name());
...
if (start > 1 && q != null) {
  ...
  out.println("<button name=\"prev\" type=\"submit\" onclick=\"this.form['start'].value="+n+"\">◀</button>");
    ...
    %>
      ...
      <input type="hidden" id="q" name="q" value="<%=JspTools.escUrl(q)%>"/>
               <%
if (topDocs != null) {
  long max = topDocs.totalHits.value;
  out.println("<input  name=\"start\" value=\""+start+"\" autocomplete=\"off\" class=\"start\"/>");
